...
class ChangeViewV2 extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    NULL AS `site_code`,
    NULL AS `site_site_clearance_type`,
    NULL AS `site_processing_status`,
    NULL AS `site_licence_no`,
    NULL AS `site_session_deleted_at`,
    NULL AS `site_submit_date`,
    NULL AS `site_issue_date`,
    NULL AS `site_expire_date`,
    NULL AS `site_rejected_date`,
    NULL AS `site_status`,
    NULL AS `site_count`,
    NULL AS `site_deleted_at`,
    NULL AS `assistance_first_name`,
    NULL AS `assistance_last_name`,
    NULL AS `officer_first_name`,
    NULL AS `officer_last_name`
FROM
    `clients`
...
JOIN `e_p_l_s` ON `clients`.`id` = `e_p_l_s`.`client_id`
WHERE
...
SELECT
    ...
    NULL AS `epl_id`,
    NULL AS `epl_code`,
    NULL AS `epl_issue_date`,
    NULL AS `epl_expire_date`,
    NULL AS `epl_submitted_date`,
    NULL AS `epl_certificate_no`,
    NULL AS `epl_count`,
    NULL AS `epl_status`,
    NULL AS `epl_rejected_date`,
    NULL AS `epl_deleted_at`,
...
ON
    `assistant_directors`.`user_id` = `as_users`.`id`
JOIN `site_clearence_sessions` ON `clients`.`id` = `site_clearence_sessions`.`client_id`
JOIN `site_clearances` ON `site_clearence_sessions`.`id` = `site_clearances`.`site_clearence_session_id`
WHERE
    `environment_officers`.`active_status` = 1 AND `assistant_directors`.`active_status` = 1 AND `clients`.`deleted_at` IS NULL';
