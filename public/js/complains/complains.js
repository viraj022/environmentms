...
function getJtableBtnHtml(full) {
    ...
    if (full["status"] != 1) {
        html +=
            ' <button type="button" class="btn btn-sm btn-primary btn-edit mr-1" value="' +
            full["id"] +
            '" data-toggle="tooltip" title="Edit"><i class="fas fa-edit" aria-hidden="true"></i></button>';
    }
    html +=
        ' <a href="/complain_profile/id/' +
        full["id"] +
        '" class="btn btn-sm btn-success mr-1" role="button" data-toggle="tooltip" title="profile view"><i class="fa fa-info-circle" style="width: 10px" aria-hidden="true" alt="profile"></i></a>';
    if (full["status"] != 1) {
        html +=
            ' <button type="button" class="btn btn-sm btn-danger btn-del" value="' +
            full["id"] +
    ...
    ulploadFileWithData(
        ...
        function (resp) {
            if (resp.status == 1) {
                ...
                swal.fire(
                    "success",
                    "Successfully save the complains",
                    "success"
                );
            } else {
                swal.fire(
                    "failed",
                    "Complain saving is unsuccessful",
                    "warning"
                );
            }
        },
    ...
    ulploadFileWithData(
        ...
        function (resp) {
            if (resp.status == 1) {
                ...
                swal.fire(
                    "success",
                    "Successfully update the complains",
                    "success"
                );
            } else {
                swal.fire(
                    "failed",
                    "Complain updating is unsuccessful",
                    "warning"
                );
            }
        },
