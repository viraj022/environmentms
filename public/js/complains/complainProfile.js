function update_attachments() {
    ...
    if (arr.length != 0) {
        ulploadFileWithData(
            ...
            function (resp) {
                if (resp.status == 1) {
                    ...
                    swal.fire(
                        "success",
                        "Successfully change the attachments of complains",
                        "success"
                    );
                } else {
                    swal.fire(
                        "failed",
                        "Complain attachments change is unsuccessful",
                        "warning"
                    );
                }
            },
        ...
        swal.fire(
            "failed",
            "One or more images must be selected to upload",
            "warning"
        );
    }
}
...
function loadProfileData(user_id, callBack) {
    ...
    ajaxRequest("GET", url, null, function (resp) {
        ...
        if (resp.letters[last_index] != null) {
            if (
                resp.letters[last_index].status == "COMPLETED" &&
                resp.status != 1 &&
                resp.status != -1
            ) {
                $("#confirm").removeClass("d-none");
        ...
        $("#comp_by").html(
            '<span class="badge bg-success">' + resp.complainer_name + "</span>"
        );
        let image = "";
        if (
            resp.attachment != null ||
            resp.attachment.length != 0 ||
            resp.attachment.file_path != null
        ) {
            ...
            if (unescaped_data != "") {
                ...
                $.each(data, function (key, value) {
                    ...
                    let file_type = file_path.substr(
                        file_path.indexOf(".") + 1
                    );
        ...
        if (resp.assigned_user != null) {
            assigned_user =
                resp.assigned_user.first_name +
                " " +
                resp.assigned_user.last_name;
        }
...
function iterateComplain_minit(object, user_id) {
    ...
    $.each(object, function (key, value2) {
        ...
        let chat_name =
            value2.minute_user.id == user_id ? "float-right" : "float-left";
        let chat_time =
            value2.minute_user.id == user_id ? "float-left" : "float-right";
...
function iterateComplain_comments(object, user_id) {
    ...
    $.each(object, function (key, value2) {
        ...
        let chat_name =
            value2.commented_user.id == user_id ? "float-right" : "float-left";
        let chat_time =
            value2.commented_user.id == user_id ? "float-left" : "float-right";
    ...
    ajaxRequest("GET", url, null, function (resp) {
        ...
        $.each(resp, function (key, value2) {
            ...
            if (value2["assignee_user"] != null) {
                assignee_name =
                    value2["assignee_user"].first_name +
                    " " +
                    value2["assignee_user"].last_name;
            }
            if (value2["assigner_user"] != null) {
                assigner_name =
                    value2["assigner_user"].first_name +
                    " " +
                    value2["assigner_user"].last_name;
            }
            forward_hist_table +=
                "<tr><td>" +
                key +
...
function load_user_by_level(level) {
    ajaxRequest(
        "GET",
        "/api/get_users_for_level/level/" + level,
        null,
        function (dataSet) {
            var combo = "";
            if (dataSet.length == 0) {
    ...
    ajaxRequest(
        ...
        function (result) {
            if (result.status == 1) {
                swal.fire(
                    "success",
                    "Successfully assigned the user to the complain",
                    "success"
                );
                ...
                swal.fire(
                    "failed",
                    "User assigning was unsuccessful",
                    "warning"
                );
            }
        }
        ...
        ajaxRequest(
            ...
            function (result) {
                if (result.status == 1) {
                    ...
                    swal.fire(
                        "failed",
                        "Comment addition was unsuccessful",
                        "warning"
                    );
                }
            }
    ...
    if (data.minute != "") {
        ajaxRequest("POST", "/api/minute_on_complain", data, function (result) {
            if (result.status == 1) {
                ...
                swal.fire(
                    "failed",
                    "minute addition was unsuccessful",
                    "warning"
                );
            }
        });
...
function confirm_complain(complain_id) {
    ajaxRequest(
        "GET",
        "/api/confirm_complain/complain/" + complain_id,
        null,
        function (result) {
            if (result.status == 1) {
                swal.fire(
                    "success",
                    "Successfully confirmed the complain",
                    "success"
                );
                loadProfileData();
            } else {
                swal.fire(
                    "failed",
                    "Complain confirmation was unsuccessful",
                    "warning"
                );
            }
        }
...
function reject_complain(complain_id) {
    ajaxRequest(
        "GET",
        "/api/reject_complain/complain/" + complain_id,
        null,
        function (result) {
            if (result.status == 1) {
                swal.fire(
                    "success",
                    "Successfully rejected the complain",
                    "success"
                );
                loadProfileData();
            } else {
                swal.fire(
                    "failed",
                    "Complain rejection was unsuccessful",
                    "warning"
                );
            }
        }
...
function forward_letter_preforation(complain_id) {
    ajaxRequest(
        "GET",
        "/api/forward_to_letter_preforation/complain/" + complain_id,
        null,
        function (result) {
            if (result.status == 1) {
                swal.fire(
                    "success",
                    "Forward to letter preforation was successful",
                    "success"
                );
                loadProfileData();
            } else {
                swal.fire(
                    "failed",
                    "Forward to letter preforation was unsuccessful",
                    "warning"
                );
            }
        }
...
function assign_file_no(complain_id, client_id) {
    ...
    ajaxRequest("POST", url, data, function (resp) {
        if (resp.status == 1) {
            swal.fire(
                "success",
                "File No has assigned for the complain",
                "success"
            );
        } else {
            swal.fire(
                "failed",
                "File No assignment for the complain was unsuccessful",
                "warning"
            );
        }
    });
