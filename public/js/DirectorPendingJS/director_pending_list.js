...
function loadDirectorPendingListTable() {
    getDirectorPendingList(function (result) {
        var dataObj = {
            ...
            3: "AD Certificate Pending Approval",
            4: "D Certificate Approval Peniding",
            5: "Complete",
            6: "Issued",
            "-1": "Rejected",
            "-2": "Hold",
        ...
        if (result.length == 0) {
            ...
            $.each(result, function (index, row) {
                ...
                tbl +=
                    '<td class="">' +
                    cer_type_status[row.cer_type_status] +
                    "(" +
                    fixMydate +
                    ")</td>";
                ...
                if (row.file_status != 0) {
                    tbl +=
                        '<td><button value="' +
                        escape(JSON.stringify(row)) +
                        '" class="btn btn-success actionDetails">Action</button></td>';
                } else {
                    tbl += "<td>N/A</td>";
                }
                tbl += "</tr>";
    ...
    ajaxRequest("GET", url, null, function (result) {
        ...
        if (result.length == 0) {
            ...
            $.each(result, function (index, row) {
                ...
                if (row.file_status != 0) {
                    tbl +=
                        '<td><a href="/certificate_perforation/id/' +
                        row.id +
                        '" class="btn btn-sm btn-success">Upload Certificate</a></td>';
                } else {
                    tbl += "<td>N/A</td>";
                }
                tbl += "</tr>";
...
function loadCertificatePathsApi(file_id, callBack) {
    ...
    ajaxRequest(
        "GET",
        "/api/files/certificate/officer/id/" + file_id,
        null,
        function (dataSet) {
            if (
                typeof callBack !== "undefined" &&
