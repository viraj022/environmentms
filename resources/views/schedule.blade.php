                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="form-group">
                                        ...
                                        <div id="valAsDirect" class="d-none">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div id="valEnvOfficer" class="d-none">
                                            <p class="text-danger">Field is required</p>
                                        </div>
                                    </div>
                                </div>
        ...
        $(function() {
            ...
            loadAssDirCombo(function() {
                loadEnvOfficerCombo($('#getAsDirect').val(), function(rest) {
                    ...
                    loadCalenderApi($('#getEnvOfficer').val(), function(
                        event) { //get all events from db
                        calendar.addEventSource(
                            event); //Passing Funtion Array Into This(event)
                    });
                });
            });
            ...
            // set inspection date in event modal
            $(document).on('click', '#setInspectionDateButton', function() {
                let client_id = $(this).val();
                let inspectionDate = $('#inspectionDate').val();
                var frmValues = {
                    "remark": "",
                    "environment_officer_id": $('#getEnvOfficer :selected').val(),
                    "schedule_date": inspectionDate
                };
                if (inspectionDate == null || inspectionDate == undefined || inspectionDate.length == 0) {
                    alert('Please Set Correct Inspection Date!');
                    return false;
                }
                personalInspectionCreateApi(client_id, frmValues, function(e) {
                    show_mesege(e);
                    loadCalenderApi($('#getEnvOfficer').val(), function(
                        event) { //get all events from db
                        setInspectionNeededApi($('#getEnvOfficer').val(), false);
            ...
            $("#getEnvOfficer").change(function() {
                ...
                loadCalenderApi($('#getEnvOfficer').val(), function(event) { //get all events from db
                    calendar.getEventSources()[0]
                        .remove(); // This will remove all events from calender
                    calendar.addEventSource(event); // This will add all events
                });
            });
            ...
            var calendar = new Calendar(calendarEl, {
                ...
                eventReceive: function(info) {
                    ...
                    var returnDate = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date
                        .getDate(); //Format Date Manually
                    var frmValues = {
                        "remark": "",
                        "environment_officer_id": env_officer_id,
                    ...
                    personalInspectionCreateApi(info.event.id, frmValues, function(rep) {
                        show_mesege(rep);
                        if (rep.message == 'true') {} else {
                            inspectionsByDateAPI($(this).data('date'), $('#getEnvOfficer')
                                .val(),
                                function() {});
                        }
                    });
            ...
            $(document).ready(function() {
                $(document).on('click', '.fc-past,.fc-future,.fc-today',
                    function() { // Remove ".fc-future" to prevent clicking future dates
                        inspectionsByDateAPI($(this).data('date'), $('#getEnvOfficer').val(),
                            function() {});
                        $('#modal-xl').modal('show');
            ...
            $(document).on('click', '.removeInspecBtn', function() {
                InspectionRemoveApi($(this).val(), function(resp) {
                    ...
                    calendar.removeAllEvents(); //clear calender before adding events
                    if (resp.id == 1) {
                        inspectionsByDateAPI($('#modalTitle').data('inspecdate'), $(
                            '#getEnvOfficer').val(), false);
                        setInspectionNeededApi($('#getEnvOfficer').val(), false);
                        loadCalenderApi($('#getEnvOfficer').val(), function(
                            event) { //get all events from db
                            calendar.getEventSources()[0]
