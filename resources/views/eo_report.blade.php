                                ...
                                <div class="row">
                                    <div class="col-4">
                                        <div class="form-group">
                                            <label>Industry Category</label><input id="is_industry"
                                                name="industry_category_check" class='ml-3' type="checkbox" />
                                            <select id="industry_cat" name="industry_category"
                                                class="form-control form-control-sm">
                                                <option value="0">Loading..</option>
                                            </select>
                                        </div>
                                        <button type='button' id='report_generate' class='btn btn-primary'>Generate
                                            Report</button>
                                    </div>
                                    <div class="col-4">
                                        <div class="form-group">
                                            <label>Pradeshiya Saba</label><input id="is_pradeshiya_sabha"
                                                name="pradeshiya_sabha_check" class="ml-3" type="checkbox" />
                                            <select id="pradeshiya_sabha" name="pradeshiya_sabha"
                                                class="form-control form-control-sm">
                                                <option value="0">Loading..</option>
                                            </select>
                                        </div>
    ...
    <script>
            ...
            ajaxRequest('GET', "/api/pradesheeyasabas", null, function(dataSet) {
                if (dataSet) {
                    $.each(dataSet, function(index, row) {
                        cbo += '<option data-ps_code="' + row.code + '" value="' + row.id + '">' + row
                            .code + ' - ' + row.name + '</option>';
                    });
                } else {
        ...
        function IndustryCategoryCombo(callBack) {
            ...
            ajaxRequest('GET', "/api/industrycategories", null, function(dataSet) {
                if (dataSet) {
                    $.each(dataSet, function(index, row) {
                        cbo += '<option data-cat_code="' + row.code + '" value="' + row.id + '">' + row
                            .name + '</option>';
                    });
                } else {
                    cbo = "<option value=''>No Data Found</option>";
